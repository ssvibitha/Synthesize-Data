...
# uses one hot encoding instead of label encoding
# Final code -x-
...
class MLModel:
    ...
    def fit(self):
        ...
        numeric_cols = ["NumberOfCalls", "NumberOfMeetings", "NumberOfOtherTasks"]
        ...
        ohe_features = ohe.fit_transform(data[categorical_col])
        ohe_feature_names = ohe.get_feature_names_out(categorical_col)
        ohe_df = pd.DataFrame(ohe_features, columns=ohe_feature_names, index=data.index)


        data.fillna(data.median(numeric_only=True), inplace=True)

        X = pd.concat([data[numeric_cols], ohe_df], axis=1)        
    ...
    def predict(self):
        ...
        ohe_features = ohe.transform(new_data[categorical_col])
        ohe_feature_names = ohe.get_feature_names_out(categorical_col)
        ohe_df = pd.DataFrame(ohe_features, columns=ohe_feature_names, index=new_data.index)

        new_data.fillna(new_data.median(numeric_only=True), inplace=True)

        X_new = pd.concat([new_data[numeric_cols], ohe_df], axis=1)        
